...
jQuery.extend(jQuery.fn, {
	toplinkwidth: function(){
		totalContentWidth = jQuery('.document').outerWidth(); // ширина блока с контентом, включая padding
...
jQuery(function($){
	...
	$(window).scroll(function() {
		if($(window).scrollTop() >= 1) {
			topLink.fadeIn(300).children('').html('').parent().removeClass('bottom_button').addClass('top_button');
		} else {
	...
	topLink.click(function(e) {
		if($(this).hasClass('bottom_button')){
			...
			$('html, body').animate({scrollTop: 00}, 200);
		}
		return false;
	});
});